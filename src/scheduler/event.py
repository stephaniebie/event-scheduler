...
class Event:
    ...
    def __init__(
        ...
        try:
            ...
            # NOTE: Assumes events are 1 hour long
            # TODO: This can be improved by making the time delta a parameter of the class
            self.end_time = self.start_time + datetime.timedelta(hours=1)
        except ValueError:
            raise ValueError(f"Invalid date '{self.date}' or time '{self.time}'")
